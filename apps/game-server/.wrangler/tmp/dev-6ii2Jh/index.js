...
// .wrangler/tmp/bundle-pw0eql/strip-cf-connecting-ip-header.js
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260612.1/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-console
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260612.1/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-process
...
var DT = 1 / TICK_RATE;
var ARENA_WIDTH = 1e3;
var ARENA_HEIGHT = 1e3;
var PLAYER_RADIUS = 22;
var PLAYER_SPEED = 260;
var PLAYER_MAX_HEALTH = 100;
var PLAYER_RESPAWN_TICKS = TICK_RATE * 2;
var PROJECTILE_RADIUS = 6;
var PROJECTILE_SPEED = 520;
var PROJECTILE_LIFETIME_TICKS = TICK_RATE * 2;
var PROJECTILE_DAMAGE = 25;
var FIRE_COOLDOWN_TICKS = Math.round(TICK_RATE * 0.25);
var SPAWN_POINTS = [
...
__name(neutralInput, "neutralInput");

// ../../packages/sim/src/vec.ts
function length(x, y) {
...
function step(state, inputs) {
  ...
  for (const player of players) {
    ...
    const dir3 = normalize(input.moveX, input.moveY);
    player.x = clamp(
      player.x + dir3.x * PLAYER_SPEED * DT,
      PLAYER_RADIUS,
      ARENA_WIDTH - PLAYER_RADIUS
    );
    player.y = clamp(
      player.y + dir3.y * PLAYER_SPEED * DT,
      PLAYER_RADIUS,
  ...
  for (const pr of projectiles) {
    ...
    pr.y += pr.vy * DT;
    pr.ttl -= 1;
    const outOfBounds = pr.x < 0 || pr.x > ARENA_WIDTH || pr.y < 0 || pr.y > ARENA_HEIGHT;
...
var PlayerInputSchema = external_exports.object({
  moveX: external_exports.number(),
  moveY: external_exports.number(),
  aim: external_exports.number(),
  firing: external_exports.boolean()
});
var PlayerSchema = external_exports.object({
  id: external_exports.string(),
  x: external_exports.number(),
  y: external_exports.number(),
  aim: external_exports.number(),
  health: external_exports.number(),
  fireCooldown: external_exports.number(),
  respawnIn: external_exports.number(),
  slot: external_exports.number().int(),
  score: external_exports.number().int()
});
var ProjectileSchema = external_exports.object({
  id: external_exports.number().int(),
  ownerId: external_exports.string(),
  x: external_exports.number(),
  y: external_exports.number(),
  vx: external_exports.number(),
  vy: external_exports.number(),
  ttl: external_exports.number().int()
});
var SnapshotSchema = external_exports.object({
  tick: external_exports.number().int(),
  players: external_exports.array(PlayerSchema),
  projectiles: external_exports.array(ProjectileSchema)
});
var ClientInputMessage = external_exports.object({
  t: external_exports.literal("input"),
  /** The client tick this input belongs to (for prediction/reconciliation). */
  tick: external_exports.number().int(),
  input: PlayerInputSchema
});
var JoinMessage = external_exports.object({
  t: external_exports.literal("join"),
  v: external_exports.literal(PROTOCOL_VERSION),
  roomId: external_exports.string()
});
var ClientMessage = external_exports.discriminatedUnion("t", [JoinMessage, ClientInputMessage]);
var WelcomeMessage = external_exports.object({
  t: external_exports.literal("welcome"),
  v: external_exports.literal(PROTOCOL_VERSION),
  youId: external_exports.string(),
  tickRate: external_exports.number().int()
});
var SnapshotMessage = external_exports.object({
  t: external_exports.literal("snapshot"),
  /** Echoes the last client input tick the server has applied (reconciliation). */
  ackTick: external_exports.number().int().optional(),
  snapshot: SnapshotSchema
});
var ServerMessage = external_exports.discriminatedUnion("t", [WelcomeMessage, SnapshotMessage]);
...
var GameRoom = class extends DurableObject {
  ...
  acceptConnection(socket) {
    ...
    const cleanup = /* @__PURE__ */ __name(() => this.onClose(socket), "cleanup");
...
// src/index.ts
var ROOM_PATH = /^\/rooms\/([A-Za-z0-9_-]{1,64})$/;
var src_default = {
  async fetch(request, env2) {
    ...
    const id = env2.GAME_ROOM.idFromName(roomId);
    const stub = env2.GAME_ROOM.get(id);
    return stub.fetch(request);
  }
};

// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260612.1/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260612.1/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-pw0eql/middleware-insertion-facade.js
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260612.1/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-pw0eql/middleware-loader.entry.ts
